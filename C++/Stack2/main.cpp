...
/*适配器模式:转换,把一种容器(vector/list/deque)通过封装转换为另一种容器
 * 1.接口:
 * push: push_back() 
 * pop:  pop_back() 
 * top:  back()
 ...
 * size: size()
 * 2.默认实现方式:deque 
 * 原因:(相对于vector来说)增容代价小,(相对于list来说)不会造成内存碎片
 * 3.源代码template <class T,class Container = deque<T>>
 * class stack{};
 * 4.使用:
 * stack<T>
 * stack<T,vector<T>> 我就想用vector去实现栈时可以这样写
 * stack<T,list<T>> 我就想用list去实现栈时可以这样写*/
